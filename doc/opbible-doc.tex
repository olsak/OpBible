...
of a~Study Bible  that breaks up the pages so that the notes end up on the same pages with
  the verses they're commenting on, on a system with a quad-core Ryzen 3 processor, it took about
...
users. You do not need to have \TeX/live installed; you can run it online via 
...
or even better: create the file `your-translation.out` for the entire Bible, and run `maketxs yourtranslation.out` to create all the `.txs` files needed; see also~\ref[txs].
...
The command \`\fmtkeep``{<chapter-number>:<verse-number>}{<phrase>}` makes first occurence of the <phrase> unsearchable. So you can apply a next `\fmt*` command to the next occurrence of the <phrase>.
