...
\centerline{\url{https://phos.club/}}
\vfill
{\it
...
When the Bible text is finished by the `\processbooks` command then there
can be more commands for generating final pages of the Bible before the `\bye`
command in the main file. An example of the final part of the main file
should looks like this:

\begtt
...
\processbooks % Generates books declared in \printedbooks
\vfil\supereject  % go to the next page

\nonum\sec Concordance
\concordance {ccd.tex} % printing concordance
\bye  % the end of the TeX work
\endtt

This example starts the new page with the section title \"Concordance" and
then the command \`\concordance` `{<file-name>}` reads the file <file-name>
where the concordances can be prepared in the form similar to this example:

\begtt \catcode`<=12
\ccd Jesus
Son of God
\+ Life: Genealogy  <Matt 1:1-17>; <Luke 3:21-37>.
   Birth:  <Matt 2:1-12>; <Luke 2:1-40>.
   Escape to Egypt   <Matt 2:13-23>.
\+ Miracles: Healing <Matt 4:5> ...
\+ Teaching: ... <Matt 6:7> ...
\+ Parables: ... ... ...

\ccd \x/Cyrus/
\+ Prophecied: <Isa 44:28>; <45:1>.
\+ Decree:  <Ezra 1:2-4>, <6:3-5> ...
\endtt

Each single concordance item begins with the command \`\ccd` followed by the
heading phrase of the concordance. This phrase will be printed bold and
concordances will be sorted alphabetically by this phrase. Next lines
includes additional text of the item. The command
\`\+`` <word>:` means that the <word> will be printed at the next line and it
can be formatted specially. By default, the concordances are printed in the
three columns mode like in a dictionary.

Individual items must be separated by empty lines in the source file. You need not
to write them in the alphabetical order becuase the sortig is done by
\TeX.

You can use words in more translation variats. For example `\x/Cýros/`
is printed as Cýros in the BKR variant but as Kýros in the CEP variant
(both are Czech Bibles; English translations usually do not alter the spelling of the names of the rulers).
It means that it is sorted before Jesus in BKR but after Jesus in CEP.

You can create a link to concordnce by \`\ccdlink``[<name>]{text}`,
for example\nl `\ccdlink[Jesus]{see also Jesus concordance}`.
