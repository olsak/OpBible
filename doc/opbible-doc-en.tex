...
You can write your commentary in the way that the result would be all six translations at the same time, with no need to modify the notes file, while the notes (or charts, maps, etc.), will display the phrases as they appear in your current Bible. See sample book of Daniel, chapter two, the chart of Daniel's visions.
...
(After all, what'd you expect, it's a software. Go to God for forgiveness.) As you'll read below, 
the \OpBible/ loads all notes for one entire book of the Bible into memory at a time and only then  
begins to digest these notes and piece them together into pages with their respective verses. If you 
...
\recommended Use the Linux operating system. \OpBible/, as a macro file for \TeX, will run on any 
...
Using `\load[opbible]`,  the \TeX/ loads macros of the \OpBible/ package. This is the most important 
program that takes care of the typesetting.
...
also includes the translation mark. In the \`\txsfile` macro, you can use 
 ...
 In the example shown above,  `.txs` files are  located in the `sources/` directory and are  named `<translation-tag>-<book-tag>.txs`, so for example `BBE-Gen.tex`.
...
\medskip
\centerline{\picw=.7\hsize\inspic{images/NETfreeDan-crop.pdf} }
...
\watchout
 Quotation marks in place of a phrase cannot be used in definitions of `\ww` (see~chapter~\ref[ww]) 
 that precede `\Note`. They only work in the `\vdef` definition in the variants file, to be applied 
...
weight of the materials but also in its value. The image was clearly too heavy with fragile feet. It 
is an illustration of the fate of all human kingdoms and civilizations:  at the end, each of them 
...
commenting percentage, so that we know where what belongs without groping.
...
So under `\endinput` we can have a few lines prepared, e.g., in this form:
...
`\Note`, or delete `={}` where they are not needed. This way you don't lose track of where
...
The `<list of variants>` is a single translation variant or a list of translation variants separated 
...
\endtt
prints ... see {\Blue note to Jer 7:4} for more information.
...
Sometimes the reader can determine the location of a verse from the context, therefore `<data>` does't need to be complete. In the {{\em incomplete data}}, there may be a  `<book>` missing, or
...
... see all of {\Blue verse Jer 8:13}--{\Blue 22}.
...
on~Jer~8:13.
\TeX/ knows that too, and assignes correct links (to be clicked on) to the incomplete entries
...
as if it were active, but it is not, and the \TeX/ gives
...
is incorrect. However, the first time \TeX/ runs, all
...
The first dimension (`5mm`) determines the horizontal offset on the x-axis, the second the vertical offset on the y-axis, 
with the coordinates `0mm 0mm` being the bottom left corner of the image.
...
Command \`\putBot` `<verse number> {<title>} [<label>] (<commands>) {<code>}`
behaves similarly to \`\insertBot`, but inserts the image or diagram not on the current
page, but to the page where the verse specified by its <verse number> is located.
Such commands can be used in the notes file, typically near its end.
...
In the example we have the name of the city of Jerusalem printed at an oblique angle so that it 
...
We will change the additional parameters to `\puttext` as we have already seen when placing 
Jerusalem on the map in the~\ref[slope].

The `\kern` command determines the spacing between letters; the digits in the `\pdfrotate` 
...
`\tline`, `\tlines` to create a timeline, see the section~\ref[timeline].
...
are on the same line are written below each other in any order. 
 \`\vskip` `<dimension>` command moves to the next line, where the dimension can be specified in
...
\arrowtext 30..430 (\bf) {Abraham's seed afflicted \hskip1cm in a land not theirs for 400 years (Gen 15:13)}
...
In the `templates.tex` file, the macro `opbible.opm` must first be loaded using `\input`, 
...
\endtt
%
There <filename> must (and <file-content> can) contain the double character `@@`, 
...
`%  Introduction to the book @@@` in the `templates.tex` file
...
A'} at the beginning of the lines, type `\style q` right after `\begChiasm`. If then you want them 
back, start the new chiasm with the line  `\begChiasm \style Q`. 
...
translation, e.g., BBE, RNKJV, etc. 
   Example: `\def\tmark{BBE}` `%\def\tmark{NETfree}`
...
* \`\input`: File to be read in:`\input {<file-name>}`. Example: ` \input {vars.tex} \input` 
`{books.tex}`
...
* \`\Note` `<verse number> {<commented phrase>} <note text terminated by a blank line>`:
  declares a note. Example:   `\Note 5:12 {Belteshazzar} See <"note on" 1:7>n.`  
...
Example: `\putArticle 5:20 {Who was Darius the Mede?} [6] ()`
...
...
%\enditems
*`\rightnote{Right column text of the outline}`
\enditems
...
*\`\Article` `[<chapter-number>]` `<text of the article.>`
    The `<chapter-number>` could be any number, unique for that particular article, but 
    as we suppose that there will most likely not be more than one article per chapter,
    it sounds logical to assign the number of the very chapter where we want the article to occur.
    This also ensures that you will easily remember the article's number when you call it to appear 
...
You chose between`\left` or `\right` after you make sure that all the preceding material of that 
particular book is complete (thus ensuring that the article will not later switch pages of its 
beginning from odd to even or vice versa) and then test it with `\swapCites`. When you are happy  
with the outcome, feel free to disable `\swapCites`.
...
\begitems
* \`\fmtpre` `{<chapter-number:verse-number>}{<command>}` inserts material (text or space) 
before specified verse.
* \`\fmtadd` `{<chapter-number:verse-number>}{<command>}` inserts material after specified verse.
* \`\fmtins` `{<chapter-number:verse-number>}{<phrase>}{<command>}` inserts material after the 
...
usually in situations where the chapter title is to be immediately followed by a pericope title:
...
* \`\insertBot` `{<Map's title>}[<label>](<optional parameters>){<map legend>}` 
...
Map legend is everything that will show over the image as text of any kind. 
...
* \`\puttext` `55mm 53mm {<"Dan 11:5 (“king of the south”)"_Dan 11:5>}` to place a text that is an 
   active hyperlink at the same time;
*  `\puttext` `48mm 55mm {\c[-40/\kern1pt\pdfrotate{0}]{Jerusalem}}` to place an oblique text; 
