...
\sec[clanky] Formát příkazů pro vložení dalších objektů
...
Text článků ke každé knize Bible zapisujte do souboru `articles-*.tex`, například
...
`BKR` pro překlad podle Bible Kralické nebo `CEP` pro ekumenický překlad.
...
\input {sources/Cze\tmark-books.tex}  % Book titles and marks

\def\txsfile   {sources/Cze\tmark-\bmark.txs}     % Localization of .txs files
\def\fmtfile   {formats/fmt-Cze\tmark-\bmark.tex} % Localisation of fmts
...
`\input {sources/Cze\tmark-books.tex}` přečte soubor, kde jsou pomocí
...
\begtt
\def\txsfile   {sources/Cze\tmark-\bmark.txs}     % Localization of .txs files
\def\fmtfile   {formats/fmt-Cze\tmark-\amark.tex} % Localisation of fmts
...
takovém případě neprovede nic, takže hlavní text Bible se vůbec nenačte.
