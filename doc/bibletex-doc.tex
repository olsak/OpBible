...
OP-Bible je sada nástojů (maker a dalšího podpůrného softwaru) na
...
navic bohatě prolinkovaný hypertextovými odkazy.
...
maketxs CzeBKR.out
\endtt
Uvedený příkaz kromě 66 `.txs` soborů vytvoří soubor `CzeBKR-books.tex`, ve
...
separátního adreáře projektu, například s názvem `sources/`. 
...
Povšimněmě si, že fráze se váže na překlad Bible Kralické, zatímco v
...
pužitého překladu, je popsáno v sekci~\ref[ruzne-preklady].
...
ale v poznámce je vytišteno heslo \uv{Pán vydal}.
...
V hlavním souboru lze zvolit
\begtt
\variants <počet-variant> {<varianta-A>} {<varianta-B>} {<varianta-C>} {<varianta-D>}
\setvariant {<jméno-zpracovávané-varianty>}
\endtt
a všechny variantní fráze (které jsou pro různé případy popsány níže) se přizpůsobí
variantě zvolené v `\setvarinat`. Seznam variant uvedených ve svorkách 
v parametrech příkazu `\variants` může být libovolný a musí obsahovat 
přesně tolik variant, kolik je <počet-variant>.

Variantní fráze v textech poznámek se uvedou pomocí `\x/<fráze-A>/` a
deklarují se pomocí
\begtt
\vdef {<fráze-A>} {<fráze-B>} {<fráze-C>} {<fráze-D>}
\endtt
přičemž `\vdef` musejí mít přesně tolik parametrů, kolik jich je stanoveno
jako <počet-variant> v příkaze `\variants`. 
Fráze první varianty <fráze-A> jsou ve zdrojovém textu poznámek 
skutečně použity (v příkazech `\x`), takže tuto variantu překladu 
označujeme jako \uv{výchozí}.

Uvedeme si příklad se třemi variantami: varianta `BKR` pro Kralický
překlad (výchozí), dále `CEP` pro ekumenický překlad a `CSP` pro studijní
překlad. V hlavním souboru máme
\begtt
\variants 3 {BKR} {CEP} {CSP}
\setvariant {CEP}
\endtt
Dále například v souboru `vars.tex` jsou deklarace
\begtt
%      BKR              CEP            CSP
...
\endtt
Jestliže v textu použijeme `\x/Baltazar/` pak tento text se vytiskne jako
\uv{Baltazar} pokud není `\setvariant` nastaveno vůbec nebo má parametr
`{BKR}`. Pokud ale je `\setvaraiant{CEP}`, pak `\x/Baltazar/` se
vytiskne jako \uv{Beltšasar}. Například poznámka v knize Daniel uvedená 
...
\endtt
a tato poznámka při `\setvariant{CEP}` na svém konci zní
\uv{\dots a služby králi Nebúkadnesarovi}, zatímco při
`\setvariant{CEP}` zní \uv{\dots a služby králi Nabuchodonozorovi}.

Vyhledávaná <fráze> uvedená v parametru `\Note`, tedy
\begtt
\Note <číslo-kapitoly>:<číslo-verše> {<fráze>} <text-poznámky>
\endtt
se před vyhledáním fráze ve stanoveném verši automaticky promění podle aktuální varianty
předkladu následujícím způsobem:
\begitems
* Je-li <fráze> deklarována pomocí `\wdef`, 
  použije se místo ní odpovídající variantní fráze, 
* jinak, je-li <fráze> deklarována pomocí `\vdef`, 
  použije se místo ní odpovídající variantní fráze,
* jinak, použije se přímo <fráze>.
\enditems
Syntaktické pravidlo `\wdef` je následující
\begtt
\CommentedBook {<zkratka-knihy>}
\wdef <číslo-kapitoly>:<číslo-verše> {<fráze>} {<fráze-B>} {<fráze-C>} 
\endtt
Počet deklarovaných frází (včetně první <fráze>) musí být roven 
<počtu-variant> deklarovaných v~příkaze `\variants`.
Deklarace variantní <fráze> pomocí `\wdef` se použije jen pro
vyhledávání konkrétního biblického verše, zatímco `\vdef` deklaruje varianty
frází požitelné v celém textu Bible. 

Variantní fráze deklarované pomocí `\wdef` mají rozšířenou syntaxi: může
...
(překlad BKR v~našem příkladě) se vyhledává \uv{vydal Pán} a tiskne se \uv{Pán vydal}
...
Může se stát, že v odlišném překladu než implicitním je třeba poznámku
navázat na jiné číslo verše než je uvedeno v parametru `\Note` a `\wdef`.
To lze zajistit těsně následovaným znakem `~` a
<číslem-kapitoly>:<číslem-verše> za variantní frází v parametrech `\wdef`. 
Například
\begtt
\Note 4:16 {srdce zvířecí} \x/Nebukadnesar/ byl postižen mentální ...
\endtt
a v souboru variants:
\begtt
%           BKR             CEP                  CSP
\wdef 4:16 {srdce zvířecí} {srdce zvířecí}~4:13 {srdce zvířecí}
\endtt
Příklad ukazuje výjimečnou situaci, kdy v překladu BKR (implicitním) máme
frázi \uv{srdce zvířecí} ve verši 4:16 knihy Daniel 
zatímco v překladu CEP je tatáž fráze ve verši 4:13 a tam ji má \TeX/ najít,
pokud zrovna zpracovává ekumenický překlad.
...
Specifikace odkazu má tyto mořžnosti:
...
\begtt \catcode`<=12
<13> vysiskne se 13 jako odkaz na verš 13 aktuální knihy a kapitoly
...
použita v textu knihy napřiklad při sestavování obsahu či v odkazech. V ukázce
...
pomocí příkazů `\BookException`, `\BookPre`, `\BookPost`, viz ...
...
`\processbooks` spostí cyklus přes všechny biblické knihy podle jejich `<a-značek>`
uložených v `\printedbooks`. Pro každou takovou knihu nejprve
načte soubor s poznámkami (umístěný podle `\notesfile`), dále přečte soubor
s formátovacími údaji (umístěný podle `fmtfile`) a konečně přečte základní
