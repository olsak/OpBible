...
po mnohá staletí pevné oznažení knihy, čísla kapitoly a čísla verše.
...
je číslo kapitoly a `<verš>` je číslo verše.
Takovému údaji říkáme {\em úplný údaj}. Kromě toho mohou v odkaze existovat neúplné
údaje, což popíšeme za chvíli.
...
nemusí být úplný. V~neúplném údaji může chybět `<kniha>` nebo
...
knihy Jeremjáš a že v druhém a třetím případě, že se týkají kapitoly 8 knihy
...
\begitems
* Chybí-li v údaji `<kniha>`, použije se `<kniha>` naposledy uvedeného úplného
  údaje.
* Chybí-li též `<kapitola>`, použije se kapitola z naposledy uvedeného údaje, kde je
  číslo kapitoly uvedeno.
\enditems

Toto pravidlo předpokládá, že první `<údaj>` musí být úplný. To ale není
pravda, v každé poznámce `\Note` text začíná s předpokladem, je případná
chybějící kniha se doplní aktuálně zpracovávanou knihou a chybějící kapitola
atuálně zpracovávanou kapitolou. Takže, pokud jsme v kapitole 4 knihy
Daniel, pak
\begtt \catcode`<=12
\Note 4.2 {} Viz též <8>.

\Note 4.7 {} Podívejte se také na <7:11>.
\endtt
vytvoří příslušné odkazy, v první ukázce proklik směřuje na Da 4:8 a ve druhé na Da 7:11.

Chcete-li v poznámce nejprve odkazovat na vzdálené místo (například na jinou
knihu) a poté na blízké místo (tj. do aktuálně zpracovávané knihy či
kapitoly) neúplným odkazem, je potřeba před tímto blízkým neúplným odkazem napsat
`\`. Například v knize Daniel:

\begtt \catcode`<=12
\Note 4.5 {} Myšlenka se opakuje v <Jr 3:5>. Ale také v \<8:3> najdete o tom zmínku.
\endtt
Zde druhý odkaz proklikne na Da 8:3, ale kdyby tam nebyl `\`, prokliknul by
na Jr 8:3.
...
neobjeví žádé varování. Pokud ale odkaz směžuje na neexistující verš nebo
