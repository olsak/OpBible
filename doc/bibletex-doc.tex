...
\begtt
\renum <zkratka-knihy> <č-kapitoly>:<výchozí-č-verše> = <varianta-překladu> <číslo-od>..<číslo-do>
\endtt
kde `<varianta-překladu>` je značka pro konkrétní překlad.
Místo `<výchozí-č-verše>` se použije při `\def\tmark{<varianta-překladu>}`
ve skutečnosti `<číslo-od>`.
Takové přečíslování se netýká jen
tohoto verše, ale celého úseku veršů vymezeného rozsahem
`<číslo-od>..<číslo-do>`.
