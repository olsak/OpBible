...
`<počet-variant>`). Tyto parametry jsou značkami pro konrétní překlad, např.
...
\endtt
Zde musí být stejný počet parametrů jako je `<počet-variant>` uvedený v
...
možné použít `\wdef`, což deklauje varianty překladu jen pro konkrétní verš
...
automaticky promění podle aktuální varianty `\tmark` předkladu následujícím způsobem:
\begitems
...
\begtt
\renum <zkratka-knihy> <č-kapitoly>:<výchozí-č-verše> = <varianta-překladu> <č-verše> <počet-veršů>
\endtt
kde `<varianta-překladu>` je značka pro konkrétní překlad.
Místo `<výchozí-č-verše>` se použije při `\def\tmark{<varianta-překladu>}`
ve skutečnosti `<č-verše>`.
Takové přečíslování se netýká jen
jednoho (prvního) verše, ale celého úseku veršů v délce `<počet-veršů>`.

Například:
\begtt
\renum Da 4:16 = CzeCSP 13 21
\endtt
