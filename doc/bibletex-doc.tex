...
některém objektu textu neúplný, přebírá nespecifikované informace z aktuálně
zpracovávané knihy, kapitoly, verše.

\mnote{Odkaz uvozený \code{\\}}
Je-li neúplný údaj uvozen znakem `\`, přebírá nespecifikované informace z
aktuálně zpracovávané knihy, kapitoly a verše bez ohledu, jaký údaj
předchází. Například:
\begtt \catcode`<=12
...
\mnote{Specifikátor odkazu}
Za koncovým znakem údaje `>` může těsně následovat specifikátor odkazu, což
je jedno s písmen:
\begitems
* `n` ... odkazuje se na poznámku
* `a` ... odkazuje se na článek
* `g` ... odkazuje se na glosu
* `i` ... odkazuje se na úvod
\enditems
...
\mnote{Zatajení údaje}
Odkaz vytiskne jen část mezi `"..."`, tj. nevytiskne se <údaj>
specifikující cíl odkazu, pokud mezi `"..."` a <údajem> je znak podtržítko.
Například \code{<}`"První kniha Samuelova"_1S 1:1>` vytiskne pouze text
{\Blue První kniha Samuelova}, který odkazuje na první verš této knihy.
