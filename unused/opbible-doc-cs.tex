...
ti, kdo budou OpBibli nejvíce využívat, budou zároveň těmi, kdo mají s \TeX/em nejméně zkušeností, snažili jsme se učinit obsluhu co možná nejjednodušší.
...
* {\bf Preciznost:} \begitems
  ...
  * Fráze, kterou poznámka komentuje, se zvýrazní odlišným řezem fontu (např. tučně).
   ...
   své fráze. Viz sekci~\ref[zlom-verse].
...
Vždy však musí být jeden z překladů aktivní, čili `\tmark` musí být definována. Kdybyste zapomněli napsat procento před řádek, který chcete zrušit, svět se nezboří; bude platit poslední definice, kterou \TeX\ načte a kterou předefinuje případné předchozí.
...
Pokud byste potřebovali nějaký jiný existující překlad, musíte ho dostat do formátu, použitelném pro OpBibli, tedy do podoby, v jaké jsou soubory `*.txs` v adresáři `sources/`.
S přípravou jednotlivých `.txs` knih z existujícího zdroje  Vám pomůže skript `maketxs`, viz~\ref[txs].

Pokud tvoříte zbrusu nový překlad a plánujete ho pro použití s OpBiblí, asi neuškodí ho rovnou psát po jednotlivých souborech pro každou knihu přímo v požadovaném formátu, viz taktéž~\ref[txs].
...
Každá kniha Bible má svůj soubor s poznámkami. 
Poznámky odkazují na místo v základním textu a úkolem \TeX/u je vytvořit stránky se základním textem a poznámkami k němu. 
Podrobněji o způsobu psaní souborů s poznámkami se píše v
sekci~\ref[note]. Všimněte si, že soubory s poznámkami jsou společné všem
překladům, tj. nejsou zde zvlášť soubory odlišené pomocí \`\tmark`. 
Pravidla psaní poznámek totiž umožní obsáhnout vyjadřovací možnosti různých překladů
společným zápisem, ovšem v jednom jazyku (například češtině; viz sekci \ref[preklady]). 
Chcete-li psát poznámky pro zcela jiný jazyk, je třeba založit nový projekt (nejlépe v jiném adresáři) s jinými `.txs` soubory, jinými formátovacími a poznámkovými soubory.
...
rozmístěné různě po biblických knihách (avšak nikoli v Úvodu). %\TODO: dopsat...
...
Takto definované varianty překladu musejí odpovídat definicím `\tmark` v hlavním souboru 
`main.tex` včetně velkých či malých písmen.

Dobře zvažte předem, jaký počet překladů budete chtít (měnit to později bude značně obtížné, byť 
přeci jen možné)
a hlavně pořadí překladů: Jak to bude uvedeno  v definici `\variants`, tak to bude platit pro celý 
projekt.
...
`<počet-variant>`, jsou uzavřeny do složených závorek a korespondují s~jednotlivými variantami 
překladu ve stejném pořadí, jak jsou varianty 
...
poznámkového souboru, napíšeme do textu poznámky jen `\x/Joakim/` (první varianta překladu v 
definici `\variants`)
...
uvnitř cyklu pro čtení všech knih příkazem `\processbooks`, dále
...
Protože do základního textu nechceme nijak zasahovat,\fnote{Řekněte si, že to jsou  svatá Písma, 
tudíž \uv{nedotknutelný} text. Jediné, co Vás oprávní do základního textu zasáhnout, je ta málo 
pravděpodobná možnost, že byste ve zdroji ze Swordu (nebo odkud jste ho získali) objevili chybu 
vůči tištěné verzi. Tu pak bude skutečně lepší odstranit na místě.} je třeba tyto doplňkové
...
\secc[zlom-verse] Stránkový zlom uvnitř verše  

Komentují-li různé poznámky různé fráze v jednom verši a zároveň stránkový zlom po veškerém formátování vychází mezi těmito frázemi v základním textu, pak poznámky následují své fráze na příslušné strany. Jinými slovy, stránkový zlom není determinován číslem verše, nýbrž frází, vyhledanou v základním textu a synchronizovanou s poznámkou. Viz příklad z Da 9:27, kde se takový případ vyskytuje např.  v B21; hledané fráze jsou zvýrazněny červeným orámováním:
\bigskip
\picw=200pt
...
Příkaz \`\putBot` `<číslo verše> {<titul>} [<lejblík>] (<příkazy>) {<kód>}`
se chová podobně jako \`\insertBot`, ale obrázek či diagram vloží nikoli na aktuální
stránku, ale na stránku, kde se nachází verš udaný svým <šíslem verše>.
Takové příkazy lze použít v souboru s poznámkami, například na jeho konci.
...
Pokud soubory `notes`, `fmt`, `intro` neexistují, \OpBible/ se nezlobí a
chová se stejně, jakoby byly prázdné. Možná ale budete chtít mít tyto
soubory pro všechny knihy Bible již připraveny v odpovídajících adresářích a
s krátkým úvodním obsahem. Šablony pro takové soubory si můžete
napsat do zvláštního souboru (např. `templates.tex`) a pak tento soubor
zpracovat příkazem `optex templates`. Tím vygenerujete výchozí soubory
podle specifikovaných šablon pro všechny knihy Bible.
Pokud soubor s daným jménem už existuje, zůstává zachován (tj. není
přepsán výchozím souborem) a objeví se o tom varování na termínálu a
v `log` souboru.

Nejprve potřebujete soubor `books.tex` s názvy a zkratkami všech knih Bible,
jak se o něm praví v sekci~\ref[books]. Ten může být vygenerován třeba
z Swordu skriptem `mod2tex`, viz sekci~\ref[txs].

V souboru `templates.tex` je třeba pomocí `\input` nejprve načíst makro
`opbible.opm`, dále soubor `books.tex` a potom můžete stanovit
šablony souborů příkazem \`\filegen` takto:

\begtt
\filegen {<název-souboru>}
<obsah souboru>
\endfile
\endtt
%
přitom <název souboru> musí obsahovat a <obsah souboru> může obsahovat dvojznak
`@@`, který se automaticky zamění za zkratku knihy <bmark> stanovenou v `bible.tex` a vygeneruje se
tolik výchozích souborů, kolik je v `books.tex` deklarováno titulů knih.
Napříkad `templates.tex` může vypadat takto:
...
%\Note 1:1 {} text ukončen prázdným řádkem
\endfile
\endtt
%
...
celý titul knihy. Takže po secifikaci `% Úvod do knihy @@@` dostanete
soubory s textem `% Úvod` {\tt do knihy První Mojžíšova (Genesis)}, dále
`% Úvod do knihy Druhá Mojžíšova (Exodus)` atd.
...
* \`\input``{<název souboru>}`: čtení dalšího souboru v daném místě.
...
\secc Typicky v souboru `books.tex`
...
\secc Typicky v souboru s formátovacími údaji
